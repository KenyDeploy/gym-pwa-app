...
export default function WorkoutApp() {
  ...
  const saveBodyStats = (e) => {
    ...
    const newEntry = {
      id: Date.now().toString(),
      date: todayStr,
      weight: parseFloat(newWeight) || (bodyStats[0]?.weight || 0),
      chest: parseFloat(newChest) || (bodyStats[0]?.chest || 0),
      arm: parseFloat(newArm) || (bodyStats[0]?.arm || 0),
      waist: parseFloat(newWaist) || (bodyStats[0]?.waist || 0),
      thigh: parseFloat(newThigh) || (bodyStats[0]?.thigh || 0)
    };
          ...
          <div className="space-y-4">
            ...
            <div className={`border rounded-xl p-4 space-y-4 ${bgCard}`}>
              <div className="flex justify-between items-center">
                ...
                <span className={`text-xs ${textMuted} font-medium`}>Ćwiczenie {activeExIdx + 1} z {activeSession.exercises.length}</span>
              </div>
            ...
            {activeTab === 'history' && (
              <div className="space-y-4">
                <div>
                  ...
                  <p className={`text-xs ${textMuted}`}>Kliknij dzień w kalendarzu, aby zobaczyć dokładne ciężary i serie.</p>
                </div>
